...
export function SeatMap({
  ...
  const getSeatStatus = (seat: Seat) => (seat as { status?: string }).status ?? seat.status;
  ...
  return (
    <div
      className="-mx-4 touch-pan-x space-y-6 overflow-x-auto overscroll-x-contain px-4 md:mx-0 md:overflow-visible md:px-0"
      onTouchStart={handleTouchStart}
      onTouchMove={handleTouchMove}
      onTouchEnd={handleTouchEnd}
    >
      <div
        className="inline-block min-w-min origin-center transition-transform duration-100 md:transform-none"
        style={{
      ...
      >
        ...
        <div className="relative">
          <div className="from-primary/50 h-2 rounded-t-3xl bg-gradient-to-b to-transparent" />
          <div className="text-muted-foreground mt-2 text-center text-sm">SCREEN</div>
        </div>
        ...
        <div className="space-y-2">
          {rows.map((row) => (
            <div key={row} className="flex items-center justify-center gap-2">
              <div className="text-muted-foreground w-8 text-center text-sm font-medium">{row}</div>
                ...
                {rowMap[row]
                  ...
                  .map((seat) => (
                    <button
                      ...
                      className={cn(
                        "h-8 min-h-[36px] w-8 min-w-[36px] touch-manipulation rounded border text-[10px] font-medium transition-all md:h-8 md:min-h-0 md:w-8 md:min-w-0",
                        "focus:ring-primary focus:ring-2 focus:ring-offset-2 focus:outline-none",
                        "active:scale-95",
                        getSeatColor(seat),
                        getSeatType(seat) === "COUPLE" && "w-16 min-w-[56px] md:w-16 md:min-w-0"
                      )}
